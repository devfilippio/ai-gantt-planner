...
|---|---|---|
| `OPENROUTER_API_KEY` | для настоящего чата | ключ OpenRouter для LLM-агента (`anthropic/claude-sonnet-4.5`, при ошибке — эскалация на `openai/gpt-4o`) |
...
**`api/agent.py`** — цикл tool-calling чат-агента (`run_agent_turn`). Стримит события `tool_call` → `patch` → `message` → `done`. У агента есть память диалога: прошлые реплики (`history`) сворачиваются в сообщения LLM перед новым запросом, поэтому агент не переспрашивает то, на что пользователь уже ответил. Есть инструмент `undo_last_turn` — по фразе «отмени» / «откати правку» агент откатывает план к состоянию до последней мутации. Системный промпт устойчив к произвольному вводу: любая задача, которую просят добавить, считается легитимной («купить молоко» — валидная задача); переспрашивает агент только при настоящей неоднозначности или когда просьба субъективна («сделай план красивее») и не даёт критерия для конкретной мутации. Доступ к модели спрятан за маленьким протоколом `LLM` с тремя реализациями: `OpenRouterLLM` (настоящий, качество-первым: `anthropic/claude-sonnet-4.5`, при ошибке эскалация на `openai/gpt-4o`), `MockLLM` (детерминированный матч по ключевым словам) и `default_llm()`, который выбирает `MockLLM`, когда стоит `MOCK_LLM=1` или `ENV=test`. Выбор модели подтверждён скриптом `scripts/model_bakeoff.py`: три реальных прод-сбоя на `gpt-4o-mini` (фабрикация изменения без вызова инструмента, пересказ всего плана markdown-списком, лишние update→delete→add вместо одного update) плюс полная хаос-батарея (`scripts/chaos_chat.py`, первые 8 сценариев) прогонялись против `gpt-4o-mini`, `gemini-2.5-flash`, `claude-haiku-4.5` и `gpt-4o` — ни один не набрал чистый балл (5/5 по сценариям + 8/8 по хаос-батарее): каждый либо дублировал/фабриковал мутацию, либо путал `shift_tasks` с `update_task` на просьбе «увеличь все задачи», либо ломал обмен исполнителями. `claude-sonnet-4.5` — единственный, прошедший всё 5/5 + 8/8, поэтому он теперь PRIMARY; `gpt-4o` остаётся FALLBACK-эскалацией на ошибки API.
...
| `demo.spec.ts`, `demo-scenarios.spec.ts` | те же и ещё три сценария, специально замедленные и записанные на видео (см. [Демо](#демо)) |

Оба viewport-проекта (`desktop` 1440×900, `mobile` 390×844) заданы в `frontend/playwright.config.ts` и игнорируют `demo*.spec.ts` (`testIgnore`) — эти спеки размеренные по темпу для видео, а не для быстрого CI-прогона. Третий проект, `demo-recording`, матчит `demo*.spec.ts` (`testMatch`) и включает запись видео.
...
Дополнительные сценарии — каждый отдельным gif, записан тем же способом из `frontend/e2e/demo-scenarios.spec.ts`:

| Gif | Что показывает |
|---|---|
| [`docs/demo.gif`](docs/demo.gif) | Главный сценарий сдачи: импорт Excel → массовая правка через чат → экспорт |
| [`docs/demo-chat-undo.gif`](docs/demo-chat-undo.gif) | Массовая правка словами («перенеси задачи Олега на неделю») и мгновенный откат («отмени последнее изменение») |
| [`docs/demo-modal.gif`](docs/demo-modal.gif) | Карточка задачи: мини-таймлайн, критический путь, стрелка длительности двигает бары, переход к зависимой задаче по чипу |
| [`docs/demo-add-task.gif`](docs/demo-add-task.gif) | Добавление новой задачи через чат с автоматической привязкой к предшественнику по имени |
